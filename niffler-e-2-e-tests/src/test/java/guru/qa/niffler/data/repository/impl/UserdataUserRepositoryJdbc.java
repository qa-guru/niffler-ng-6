...
public class UserdataUserRepositoryJdbc implements UserdataUserRepository {
    ...
    public void addIncomeInvitation(UserEntity requester, UserEntity addressee) {
        try (PreparedStatement ps = holder(CFG.userdataJdbcUrl()).connection().prepareStatement(
                "INSERT INTO \"friendship\" (requester_id, addressee_id, status, created_date) " +
                        "VALUES (?, ?, ?, ?)")) {
            ps.setObject(1, requester.getId()); // Запрашивающий
            ps.setObject(2, addressee.getId()); // Получатель
            ps.setString(3, FriendshipStatus.PENDING.name()); // Статус "PENDING"
            ps.setDate(4, new java.sql.Date(System.currentTimeMillis())); // Текущая дата
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    public void addOutcomeInvitation(UserEntity requester, UserEntity addressee) {
        try (PreparedStatement ps = holder(CFG.userdataJdbcUrl()).connection().prepareStatement(
                "INSERT INTO \"friendship\" (requester_id, addressee_id, status, created_date) " +
                        "VALUES (?, ?, ?, ?)")) {
            ps.setObject(1, addressee.getId()); // Получатель запроса становится инициатором исходящего запроса
            ps.setObject(2, requester.getId()); // Запрашивающий
            ps.setString(3, FriendshipStatus.PENDING.name()); // Статус "PENDING"
            ps.setDate(4, new java.sql.Date(System.currentTimeMillis())); // Текущая дата
            ps.executeUpdate();
        } catch (SQLException e) {
