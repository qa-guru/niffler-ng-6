...
class UserServiceTest {
    ...
    @Test
    void allUsersShouldReturnUsersWithSmallPhotoOnly(@Mock UserRepository userRepository) {
        ...
        byte[] smallPhoto1 = "small_photo_1".getBytes();
        byte[] smallPhoto2 = "small_photo_2".getBytes();

        List<UserWithStatus> mockUsers = List.of(
                new UserWithStatus(userId1, "user1", CurrencyValues.USD, "Full Name 1", smallPhoto1, FriendshipStatus.ACCEPTED),
                new UserWithStatus(userId2, "user2", CurrencyValues.EUR, "Full Name 2", smallPhoto2, FriendshipStatus.PENDING)
        );
        ...
        assertEquals("small_photo_1", result.get(0).photoSmall(), "First user's small photo should match");
        assertEquals("small_photo_2", result.get(1).photoSmall(), "Second user's small photo should match");
        assertNull(result.get(0).photo());
        assertNull(result.get(1).photo());

        verify(userRepository, times(1)).findByUsernameNot(eq(mainTestUserName));
    }

    @Test
    void getCurrentUserShouldReturnDefaultUserJsonInCaseNoDbEntry(@Mock UserRepository userRepository) {
        ...
        assertEquals(DEFAULT_USER_CURRENCY, result.currency());
    ...
    @Test
    void allUsersShouldCorrectHandleSearchQuery(@Mock UserRepository userRepository) {
        ...
        List<UserWithStatus> mockUsers = List.of(
                new UserWithStatus(userId1, "user1", CurrencyValues.USD, "Full Name 1", null, FriendshipStatus.ACCEPTED),
                new UserWithStatus(userId2, "user2", CurrencyValues.EUR, "Full Name 2", null, FriendshipStatus.PENDING)
        );
        ...
        assertEquals(2, result.size(), "Should return 2 users");
        assertEquals("user1", result.get(0).username(), "First user's username should match");
        assertEquals("user2", result.get(1).username(), "Second user's username should match");
    ...
    @Test
    void attemptToCreateFriendshipRequestWithItselfShouldThrowSameUsernameException(@Mock UserRepository userRepository) {
    ...
    @Test
    void acceptFriendshipRequestShouldUpdateFriendshipStatus(@Mock UserRepository userRepository) {

        userService = new UserService(userRepository);

        FriendshipEntity friendship = new FriendshipEntity();
        friendship.setRequester(secondTestUser); // Barsik отправил запрос
        friendship.setAddressee(mainTestUser);  // Dima получил запрос
